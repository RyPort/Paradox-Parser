...
func (ge *GameEvent) TypeName() string {
	return deadlock.DOTA_COMBATLOG_TYPES_name[ge.m.GetKeys()[0].GetValByte()]
}

func (ge *GameEvent) Type() deadlock.DOTA_COMBATLOG_TYPES {
	return deadlock.DOTA_COMBATLOG_TYPES(ge.m.GetKeys()[0].GetValByte())
}

func (ge *GameEvent) String() string {
	keys := ge.m.GetKeys()
	name := deadlock.DOTA_COMBATLOG_TYPES_name[keys[0].GetValByte()]
